...
public class GoldenBucketItem extends Item {
    ...
    protected ItemStack emptyBucket(ItemStack stack, PlayerEntity player) {
        ...
        ItemStack returnStack = level > 0 ? stack : new ItemStack(CUItems.GOLDEN_BUCKET.get());
    ...
    private Item getFilledBucket(Fluid fluid) {
        if (fluid == Fluids.WATER) {
            return CUItems.GOLDEN_WATER_BUCKET.get();
        } else if (fluid == Fluids.LAVA) {
            return CUItems.GOLDEN_LAVA_BUCKET.get();
        } else {
            return CUItems.GOLDEN_BUCKET.get();
        }
    }
    ...
    public int getBurnTime(ItemStack itemStack) {
        return itemStack.getItem() == CUItems.GOLDEN_LAVA_BUCKET.get() ? 20000 : super.getBurnTime(itemStack);
    }
}