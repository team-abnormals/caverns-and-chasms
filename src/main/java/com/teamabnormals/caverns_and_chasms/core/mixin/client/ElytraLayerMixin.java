...
public abstract class ElytraLayerMixin<T extends LivingEntity> {
	...
	private void getCloakTextureLocation(ElytraModel model, PoseStack poseStack, VertexConsumer vertexConsumer, int packedLight, int overlay, Operation<Void> original, PoseStack p_116951_, MultiBufferSource p_116952_, int p_116953_, T entity) {
		...
		if (stack.is(CCItems.COWL.get())) {
			...
			this.elytraModel.renderToBuffer(poseStack, vertexConsumer, p_116953_, OverlayTexture.NO_OVERLAY, i);
		} else {
			original.call(model, poseStack, vertexConsumer, packedLight, overlay);
		}
	}
}