...
public final class CCPoiTypes {
	...
	public static final DeferredHolder<PoiType, PoiType> LIGHTNING_ROD = POI_TYPES.register("lightning_rod", () -> new PoiType(CCBlocks.HELPER.getDeferredRegister().getEntries().stream().filter(block -> block.get() instanceof LightningRodBlock).map(block -> block.get().getStateDefinition().getPossibleStates()).collect(HashSet::new, HashSet::addAll, HashSet::addAll), 0, 1));
}