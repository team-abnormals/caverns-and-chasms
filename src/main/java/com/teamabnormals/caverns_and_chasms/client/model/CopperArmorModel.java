...
public class CopperArmorModel<T extends LivingEntity> extends HumanoidArmorModel<T> {
	...
	public static MeshDefinition createBodyLayer(CubeDeformation deformation) {
		...
		partDefinition.getChild("head").addOrReplaceChild("base", CubeListBuilder.create().texOffs(0, 0).addBox(-1.0F, -9.0F + baseY, -1.0F, 2.0F, 1.0F, 2.0F, base), PartPose.ZERO);
		partDefinition.getChild("head").addOrReplaceChild("top", CubeListBuilder.create().texOffs(24, 0).addBox(-2.0F, -12.0F + topY, -2.0F, 4.0F, 3.0F, 4.0F, top), PartPose.ZERO);
