...
public class TurquoiseBlock extends Block {
	...
	public void stepOn(Level level, BlockPos pos, BlockState state, Entity entity) {
		if (level.isClientSide() && (entity.getDeltaMovement().x > 0 || entity.getDeltaMovement().z > 0)) {
			...
			ParticleUtils.spawnParticlesOnBlockFace(level, pos, state.is(CCBlocks.TURQUOISE_BLOCK.get()) ? CCParticleTypes.TURQUOISE_BLUE_STEP.get() : CCParticleTypes.TURQUOISE_GREEN_STEP.get(), ConstantInt.of(1), Direction.UP, supplier, 0.55D);
		}
	}
}