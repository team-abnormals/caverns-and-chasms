	...
	protected void onTake(Player player, ItemStack p_150475_) {
		...
		this.access.execute((level, pos) -> {
			...
			if (state.is(CCBlocks.BEJEWELED_ANVIL.get())) {
				...
				SpinelBoom boom = new SpinelBoom(level, null, pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D, 2.0F);
				if (!ForgeEventFactory.onExplosionStart(level, boom)) {
					boom.explode();
					boom.finalizeExplosion(true);
					CavernsAndChasms.CHANNEL.send(PacketDistributor.DIMENSION.with(level::dimension), new S2CSpinelBoomMessage(pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F, 2.0F, boom.getToBlow()));
				}
			} else {
